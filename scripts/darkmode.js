...
class ThemeEffects {
    ...
    updateAnimations(theme) {
        ...
        if (heroShape) {
            if (theme === 'dark') {
                heroShape.style.background = 'linear-gradient(135deg, #1E3A8A, #3B82F6)';
            } else {
                heroShape.style.background = 'linear-gradient(135deg, #86EFAC, #22C55E)';
            }
        }
    }
}
...
function initThemeAwareComponents() {
    ...
    const cards = document.querySelectorAll('.industry-card, .service-card, .reason-card');
    
    ThemeUtils.onThemeChange((theme) => {
        cards.forEach(card => {
            // Add subtle theme-specific hover effects
            if (theme === 'dark') {
                card.style.setProperty('--hover-glow', '0 0 20px rgba(34, 197, 94, 0.2)');
            } else {
